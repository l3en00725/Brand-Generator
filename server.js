...
app.post('/api/generate-assets', async (req, res) => {
  try {
    ...
    assets.set('logos/logo-lockup-horizontal.png', await createLockup(sharp, logoBuffer, 1200, 300, 'horizontal', finalStrategy));
    assets.set('logos/logo-lockup-stacked.png', await createLockup(sharp, logoBuffer, 800, 600, 'stacked', finalStrategy));
...
- logo-lockup-horizontal.png: Icon + name lockup (horizontal)
- logo-lockup-stacked.png: Icon + name lockup (stacked)
...
async function createLockup(sharpLib, logoBuffer, width, height, layout, strategy) {
  const brandName = String(strategy?.brandName || 'Brand').trim();
  const subcopy = socialSubcopy(strategy);
  const primary = String(strategy?.colors?.primary || '#0ea5e9');
  const hex = primary.replace('#', '');
